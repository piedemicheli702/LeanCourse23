...
example {s : Set ℕ} : s ∈ atTop ↔ ∃ N, ∀ n ≥ N, n ∈s := by exact mem_atTop_sets
...
example {s t : Set ℝ} : t ∈ 𝓟 s ↔ s ⊆ t := by exact mem_principal
...
example {X Y Z : Type*} {F : Filter X} {G : Filter Y} {H : Filter Z}
    ...
    Tendsto (g ∘ f) F H := by {
      rw [Tendsto]
      have h₀ : map f F ≤ G := by exact hf
      have h₁ : map g G ≤ H := by exact hg
      rw [@le_def]
      intro x hx
      apply h₁ at hx
      apply h₀ at hx
