...
example (a b : ℝ) : ∫ x in a..b, 2 * x * exp (x ^ 2) =
  ...
  have h1 : ∀ x ∈ [[a, b]], HasDerivAt
    ...
    (2 * x * exp (x ^ 2)) x
  · intro x hx
    rw [show 2 * x * exp (x ^ 2) = (x ^ 2) * (2 * x) by
    sorry]
...
example (a b : ℝ) (h : a ≤ b) : (volume (Icc a b)).toReal = b - a := by
  simp [h]
...
example (a b : ℝ) :
    ...
    2 * cos (a / 2 + 3) - 2 * cos (b / 2  + 3) := by {
      ...
      · apply Continuous.intervalIntegrable
        continuity
      · use (fun x ↦ ((-1) * 2) * cos (x / 2 + 3))
      · simp
        ring
      · intro x hx
        apply HasDerivAt.comp (h₂ := fun x ↦ -1 * 2 * cos x) (h:= fun x ↦ x / 2 + 3) (h₂' := fun x ↦ 2 * sin x) (h' := 1 / 2)
    }
