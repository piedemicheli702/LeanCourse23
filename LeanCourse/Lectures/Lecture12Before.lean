...
example (f : E →L[𝕜] F) (x : E) : ‖f x‖ ≤ ‖f‖ * ‖x‖ := by
  apply f.le_op_norm x
...
example {f : ℝ → ℝ} (hf : Continuous f) (h2f : Injective f) {a b x : ℝ}
  ...
  have h₀ : Icc (f a) (f b) ⊆ f '' Icc a b := by
    apply intermediate_value_Icc hab
